...
for ticker, data in sheets_dict.items():
    ...
    for year in range(2012, 2023):
        ...
        valid_data = year_data.dropna(subset=['e/p'])

       
        if not valid_data.empty:
            median_value = valid_data['e/p'].median()
            if median_value < 0:
...
file_name_for_all = "../data/all_data_with_ep.xlsx"
...
plt.xlabel('Год')
plt.ylabel('Медианный E/P (%)')
plt.title('Сравнение медианных E/P по годам')
