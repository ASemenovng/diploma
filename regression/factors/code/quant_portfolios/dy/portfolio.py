...
def process_and_assign_tickers_to_quantiles_quarterly(file_name):
    ...
    for sheet_name, data in sheets_dict.items():
        ...
        for index, row in quarterly_data.iterrows():
            ...
            capitalization = row['div_rate']

            if pd.isna(capitalization):
                continue
...
def extend_portfolios_to_months(portfolios):
    ...
    for year in range(start_year, end_year + 1):
        for month in range(1, 13):
            ...
            previous_month_year = datetime(previous_year, 12, 1).strftime('%Y-%m')  
            ...
            for ticker in tickers:
                ...
                if ticker_data is not None:
                    ...
                    if not month_data.empty:
                        ...
                        return_rate = (
                                month_data['exchange_rate'].fillna(0) / 100 + month_data['div_rate'].fillna(0)).sum()
