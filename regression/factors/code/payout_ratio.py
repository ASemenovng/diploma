...
for ticker, df in sheets_dict.items():
    ...
    df_valid = df_dec[df_dec['payout_ratio'].notna()]
    all_payouts.append(df_valid[['year_month', 'payout_ratio']])


all_payout_data = pd.concat(all_payouts, ignore_index=True)


median_payout_per_year = all_payout_data.groupby('year_month')['payout_ratio'].median()


median_payout_dict = median_payout_per_year.to_dict()


print("Медианные значения payout_ratio за каждый декабрь:")
...
for ticker, df in sheets_dict.items():
    ...
    for year in range(2012, 2023):
        ...
        if not december_data.empty and pd.notna(december_data['dividend_payout_RUB'].values[0]):
            ...
            payout_ratio = december_data['payout_ratio'].values[0]

            if pd.notna(payout_ratio):
                
                for month in range(1, 13):
                    if median_value is not None:
                        if payout_ratio >= median_value:
...
print("High payout_ratio portfolio:")
for key, value in sorted(high_div_portfolio.items()):
    print(f"{key}: {value}")

print("\nLow payout_ratio portfolio:")
...
print("Доходности для портфеля с payout_ratio выше или равной медианной:")
for date, returns in high_returns.items():
    print(f"{date}: {returns}")

print("\nДоходности для портфеля с payout_ratio ниже медианной:")
...
print("Средняя доходность для портфеля с payout_ratio выше или равной медианной:")
for date, avg_return in average_high_returns.items():
    print(f"{date}: {avg_return:.2f}")

print("\nСредняя доходность для портфеля с payout_ratio ниже медианной:")
...
data = {
    ...
    'High pr Returns': list(average_high_returns.values()),
    'Low pr Returns': [average_low_returns[date] for date in average_high_returns.keys()]  
}

df = pd.DataFrame(data)


output_filename = "../data/no_div_weight_pr_factors_bc.xlsx"
