        ...
        for ticker in tickers:
            if ticker in sheets_dict:
                ...
                if not specific_row.empty:
                    ...
                    returns_dict[date].append(exchange_rate_frac)
    return dict(returns_dict)
...
print("Средняя доходность для портфеля с капитализацией выше или равной медианной:")
for date, avg_return in average_high_returns.items():
    print(f"{date}: {avg_return:.2f}")

print("\nСредняя доходность для портфеля с капитализацией ниже медианной:")
...
output_filename = "../data/no_div_weight_smb_factors_bc.xlsx"
