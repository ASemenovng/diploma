...
def prepare_data(company_file, market_file):
    ...
    for ticker, data in company_data_dict.items():
        ...
        merged_data['SMB'] = merged_data['SMB'] * 100
        merged_data['SOE'] = merged_data['SOE'] * 100

        
        merged_data = merged_data[['year_month', 'Y', 'RmRf', 'SMB', 'SOE']]
...
prepared_data = prepare_data('../data/upt_data_result.xlsx', '../data/Факторы-4.xlsx')
print(prepared_data)
...
X = sm.add_constant(fixed_effects_data[['RmRf', 'SMB', 'SOE']])  


fe_model = sm.OLS(fixed_effects_data['Y'], X).fit(cov_type='HC3')
